...
def voronoi_polygons(cells):
    ...
    for pIdx, lineIndices_ in cells.items():
        ...
        if len(dangling_lines) == 2:
            ...
            connected = list(
                filter(lambda i12_: (i12_[0], i12_[1]) != (i11, i12) and (i12_[0] == i11 or i12_[1] == i11),
                       lineIndices_))
            i11Unconnected = len(connected) == 0

            connected = list(
                filter(lambda i12_: (i12_[0], i12_[1]) != (i21, i22) and (i12_[0] == i21 or i12_[1] == i21),
                       lineIndices_))
...
if __name__ == '__main__':
    ...
    random_pct = 0.090
    P = 1000 * random_pct * np.random.random((100, 2))
    for i, p in enumerate(P):
        row = math.floor(i / 10)
        col = i % 10

        p[0] += (col * 100 - (random_pct * 100)) + 5
        p[1] += (row * 100 - (random_pct * 100)) + 5
    ...
    for poly in polys:
        ...
        for i, line in enumerate(poly):
            lines.append(line[0])
            lines.append(line[1])
        draw.polygon(lines, fill=np.random.choice(
            ['#0000ff', '#0000ee', '#0000dd', '#0000cc', '#0000bb', '#0000aa', '#000099', '#000088', '#000077',
             '#000066', '#000055', '#000044', '#000033', '#000022', '#000011']))
        ...
        for i, line in enumerate(poly):
            ...
            draw.line((line[0], line[1], l2[0], l2[1]), fill='#000011', width=3)
            draw.line((line[0], line[1], l2[0], l2[1]), fill='white')

    circ = 2
    for point in P:
        p = (int(point[0]), int(point[1]))
        draw.ellipse((p[0] - circ, p[1] - circ, p[0] + circ, p[1] + circ), fill='yellow', outline='blue')
